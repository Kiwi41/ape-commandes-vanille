...
- **📄 Générer aussi un fichier PDF**
  - Nécessite `weasyprint` installé (voir [INSTALL_PDF.md](INSTALL_PDF.md))
  - Génère HTML + PDF
  
- **📄 Générer uniquement le PDF**
  - Disponible seulement si l'option PDF est cochée
  - Génère uniquement le PDF (pas de HTML)
  
- **🌐 Ouvrir automatiquement le fichier généré**
  - Ouvre le fichier dans votre navigateur/lecteur PDF par défaut
...
- Installez `weasyprint` : voir [INSTALL_PDF.md](INSTALL_PDF.md)
- Ou décochez l'option PDF pour générer seulement le HTML
